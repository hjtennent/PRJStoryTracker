  ...
  () => {
    if (storyLink != storyInputPlaceholder && storyLink != "") {
      ...
      getStoryDetails(storyLink).then(async (response) => {
        const storyObject = getStoryObject(response["ID"], response["URL"],
                              response["TITLE"], response["STORY"], 
                              response["AUTHORS"], response["DATE"],
                              response["KEYWORDS"])
        setStory(storyObject)
        setIsStoryLoaded(true)
        //Call Firebase to save topic to database
        const firebaseResponse = await addTopic(user.uid, storyObject["url"],
                                  storyObject["title"], storyObject["authors"],
                                  storyObject["keywords"], storyObject["date"])
  ...
  const displaySavedNotifications = async () => {
    ...
    if (notifications != null) {
      notifications.forEach(async notification => {
        ...
        Alert.alert(
          parsedNotification['notification']['title'], 
          parsedNotification['notification']['body'] + "\n" + headline,
          [
            {
              text: 'Read',
              onPress: () => props.navigation.navigate('StoriesStack', {
                ...
                params: { 
                  storyID: storyID, 
                  storyUpdates: JSON.parse(dictionary[0][1]),
    ...
    //get rid of the saved notifications once they've been shown to the user
    clearData();
  }
  ...
  const getBiasAnalysis = async () => {
    ...
    if (bias['STATUS'] == 200) {
      if (bias['SOURCE'] != null) {
        ...
        Alert.alert("Bias Analysis", "We couldn't find an author you tend to" + 
            " read more than others. Good job, this suggests your news " + 
            "consumption is relatively balanced!");
      }
    } else {
      Alert.alert("Bias Analysis", "Sorry, we don't have enough data to " + 
            "give a bias analysis yet.")
    }
  }
    ...
    <>
      <View style={styles.container}>
        <View style={styles.logoutContainer}>
            <StoryButton makeHighlight={false} text={"Logout"} 
                  onPress={() => logout()} />
            <StoryButton makeHighlight={false} text={"User Data"} 
                  onPress={() => props.navigation.navigate('UserData')} />
            <StoryButton makeHighlight={false} text={"Get Bias"} 
                  onPress={() => getBiasAnalysis()} />
        </View>
        <View style={styles.welcomeContainer}>
          <Text testID={"mainTitle"} style={styles.mainText}>
            Welcome to the Story Tracker, {user && user.email}
          </Text>
        </View>
        <View style={styles.linkInputContainer}>
          <TextInput style={styles.topicLinkInput} 
                onChangeText={text => onEnterStoryLink(text)} value={storyLink} />
          <StoryButton makeHighlight={!isStoryLoaded} 
                onPress={() => retrieveStory()} 
                text={"Get Story Details"} />
        </View>
        {isStoryFollowed &&
          <View style={styles.alreadyFollowedStoryContainer}>
            <Text style={styles.alreadyFollowedText}>
              You already follow this story, 
              head to the Stories tab to look for any updates.
            </Text>
            <StoryButton makeHighlight={true} text={"Followed Stories"} 
                  onPress={() => navigateStoriesTab()} />
          </View>
        }
        {!isStoryFollowed && isStoryLoaded && 
          <View style={styles.storyContainer}>
            ...
            <View style={styles.buttonContainer}>
              <StoryButton makeHighlight={!isStoryLoaded} text={"Get Updates"}
                    onPress={() => getUpdatesButton()} />
              <StoryButton makeHighlight={false} text={"Cancel"} 
                    onPress={() => cancel()} />
            </View>
          </View>
        }
